...
drop trigger if exists set_cms_packages_updated_at on public.cms_packages;
create trigger set_cms_packages_updated_at
before update on public.cms_packages
for each row execute function public.set_updated_at();

drop trigger if exists set_cms_gallery_images_updated_at on public.cms_gallery_images;
create trigger set_cms_gallery_images_updated_at
before update on public.cms_gallery_images
for each row execute function public.set_updated_at();

drop trigger if exists set_cms_blog_posts_updated_at on public.cms_blog_posts;
create trigger set_cms_blog_posts_updated_at
before update on public.cms_blog_posts
for each row execute function public.set_updated_at();

drop trigger if exists set_cms_faqs_updated_at on public.cms_faqs;
create trigger set_cms_faqs_updated_at
before update on public.cms_faqs
for each row execute function public.set_updated_at();

drop trigger if exists set_cms_testimonials_updated_at on public.cms_testimonials;
...
-- Depois crie o utilizador em Authentication > Users com este mesmo email.
